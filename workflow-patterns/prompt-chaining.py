# Prompt chaining eocmposes a tasks into a sequence of steps, where each llm calls the previous one
import os
import logging
...
class EventExtraction(BaseModel):
    ...
    description: str = Field(description="Raw descripion of the event")
...
def process_calendar_request(user_input: str) -> Optional[EventConfirmation]:
    ...
    # Gate check: Verify if it is a calendar event with sufficient information
    if(
    ...
    # Second LLM call: Getting detailed information
    event_details = parse_event_details(initial_extraction.description)

    # Third LLM call: generate confirmation
    confirmation = generate_confirmation(event_details)

    logger.info("Calendar processing request completed successfully")
    return confirmation
...
# Step 5: Test the chain with a valid input
